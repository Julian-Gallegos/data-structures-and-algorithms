...
const sortByChildren = (charArray) => {
  ...
  return charArray.sort((a,b) => {
    return a.children.length > b.children.length ? 1 :
      a.children.length !== b.children.length ? -1 :
        a.name > b.name ? 1 :
          -1;
  });
};
...
const isCapitalized = (str) => {
  ...
  wordArray = wordArray.map(word => word.replace(/\W|_/g, ''));
...
const citiesAtoJ = (arr) => {
  // Solution code here...
  return arr.filter(city => /^[A-J]/g.test(city));
};
