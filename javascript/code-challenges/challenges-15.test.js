...
const screenForNames = (arr) => {
  // Solution code here...
  return arr.filter(str => /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z]+[A-Za-z\s]+?/.test(str));
}
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let lukeMass = 77; // Could also just fetch the first object in arr.
  let retString = '';
  let hyphenCheck = false;
  arr.forEach(obj => {
    if (Number(obj.mass) > lukeMass) {
      if (hyphenCheck) {
        retString += ` - ${obj.name}`;
      } else {
        hyphenCheck = true;
...
const isSecure = (url) => {
  // Solution code here...
  return /^https:\/\/(www\.)?\w+\.[a-z]{3}/.test(url);
};
...
const helpCheck = (pos1,pos2,pos3) => {
  return pos1 === pos2 && pos1 === pos3 && pos1 !== '';
};
const detectTicTacToeWin = (board) => {
  // Solution code here...
  if (helpCheck(board[0][0], board[0][1],board[0][2])) {
    return true;
  }
  if (helpCheck(board[1][0], board[0][1],board[0][2])) {
    return true;
  }
  if (helpCheck(board[2][0], board[0][1],board[0][2])) {
    return true;
  }
  if (helpCheck(board[0][0], board[1][0],board[2][0])) {
    return true;
  }
  if (helpCheck(board[0][1], board[1][1],board[2][1])) {
    return true;
  }
  if (helpCheck(board[0][2], board[1][2],board[2][2])) {
    return true;
  }
  if (helpCheck(board[0][0], board[1][1],board[2][2])) {
    return true;
  }
  if (helpCheck(board[0][2], board[1][1],board[2][0])) {
    return true;
  }
  return false;
};
