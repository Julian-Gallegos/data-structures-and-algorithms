...
const evenOdd = (arr) => {
  // Solution code here...
  return arr.map(num => {
    if (typeof(num) !== 'number') { // If num is not a number
      return 'N/A';
    } else if (num % 2 === 0) {
      return 'even';
    } else { // If num is odd
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(obj => ({name: obj.stat.name, total: obj.effort+obj.baseStat})); // Parentheses wrapper so that the parser knows that the curly braces are an object initalizer
};
