...
const count = (target, input) => {
  ...
  return input.map((val) => {
    ...
    return val.reduce((accum, val2) => {
      return val2 === target ? accum+1 : accum;
    }, 0);
...
const totalSum = (input) => {
  ...
  return input.map((val) => {
    ...
    return val.reduce((accum, val2) => accum+val2);
...
let findShortest = (data) => {
  ...
  return data.filter(obj => {
    if (Number(obj.height) < lowest) {
      lowest = Number(obj.height);
      return true;
    }
    return false;
  }).filter(obj => Number(obj.height) <= lowest)[0].name;
};
